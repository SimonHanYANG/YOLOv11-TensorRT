...
class YOLOv8Detector:
    ...
    def postprocess(self, outputs):
        ...
        boxes = []
        predictions.shape[1] - 4  # 总通道数减去4个边界框参数
    ...
    def draw_boxes(self, image, boxes):
        ...
        for box in boxes:
            ...
            cv2.rectangle(
                result_image,
                (int(x1), y1_label - label_size[1] - 4),
                (int(x1) + label_size[0], y1_label),
                (0, 255, 0),
                -1,
            )
            cv2.putText(result_image, label, (int(x1), y1_label - 2), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
    ...
    for idx, img_path in enumerate(image_paths):
        ...
        try:
            ...
            with open(txt_path, "w") as f:
                h, w = image.shape[:2]
                for box in boxes:
                    x1, y1, x2, y2, _conf, class_id = box
