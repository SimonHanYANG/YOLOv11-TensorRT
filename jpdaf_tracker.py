...
class JPDATrack:
    ...
    def predict(self):
        ...
        # 计算门限椭圆
        self.gatingEllipse = self.get_error_ellipse(self.GateThreshold, (self.z_pred[0, 0], self.z_pred[1, 0]), self.S)
    ...
    def correct(self, valid_measures, betta):
        ...
        for i, measure in enumerate(valid_measures):
            error_i = np.array([[measure[0] - self.z_pred[0, 0]], [measure[1] - self.z_pred[1, 0]]], dtype=np.float32)
...
class JPDAFilter:
    ...
    def validate_measurements(self, all_measurements):
        ...
        for i, measure in enumerate(all_measurements):
            ...
            for j, track in enumerate(self.tracks):
                ...
                track.calc_euclidean_distance(measure)
    ...
    def delete_tracks(self):
        ...
        while i < len(self.tracks):
            track = self.tracks[i]
            if (
                track.gatingEllipse[1][0] >= terminate_threshold and track.gatingEllipse[1][1] >= terminate_threshold
            ) or track.consecutive_misses > 10:
                self.tracks.pop(i)
            else:
                i += 1
    ...
    def create_new_track(self, init_pt):
        """创建新轨迹."""
        self.trackID += 1
        track = JPDATrack(init_pt, self.processNoise, self.measureNoise, self.Pd, self.Pg, self.trackID)
